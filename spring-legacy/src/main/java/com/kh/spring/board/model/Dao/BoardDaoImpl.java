...
public class BoardDaoImpl implements BoardDao {
	...
	public List<Board> selectList(Map<String, Object> paramMap) {
		 ...
		 * 
		 *  1. ROWNUM, ORW_NUMBER()로 페이징 처리된 쿼리 조회하기.
		 *  SELECT *
		 *  FROM (
		 ...
		 *    - 전체 쿼리결과를 자바어플리케이션으로 가져온 후, 지정한 ㅜ이치(offset)에서
		 ...
		 *    
		 *    2번은 버전 때문에 힘듦, 1번(대용량) 3번
		 */
		// 1. Rowbounds를 활용한 페이징처리
		// 몇 번쨰 행부터 몇개를 가져올지를 지정.
		...
		paramMap.put("limit", limit); // 20, 30
