...
//원래는 MemberController에 구현하는것이 맞다.
@Controller
...
public class SecurityController {
	...
	public String enroll(@ModelAttribute Member member) {
		 ...
		 * @ModelAttribute 
		 *  - 커맨드 객체 바인딩시 사용
		 *  - model영역에서 커맨드 객체를 저장
		 * 
		 * 
		 * */
		
		
		//log.info("bcrypt : {}" , passwordEncoder);
	 ...
	 *  - @ModelAtribute로 지정한 커맨드 객체에 대해 바인딩 작업을 수행한다.
	 *  - 
	 *  
	 *  처리순서
	 *  1) 클라이언트의 요청 파라미터를 커맨드 객체 필드에 바인딩시도
	 *  2) 바인딩 과정에서 WebDataBinder가 필요한 경우 타입변환 및 유료성검사를 시행
	 *  3) 유효성 검사 결과를 저장한다. (Binding result)
	 *  
	 * 
	 * 
	 * 
	...
	public void initBinder(WebDataBinder binder) {
		...
		//타입변환 수행
		//문자열 형태의 날짜값을 Date로 변환하기.(birth-day)
		...
		//날짜형태의 값이 들어오는 경우 자동으로 date자료형으로 반환해주는 커스텀이다.
	...
	public String register(@Validated	@ModelAttribute Member member ,
			...
			//BindingResult
			// - 유효성 결과를 저장하는 객체
			// - forward시 자동으로 jsp에 전달되며 form태그 내부에 
			// - 에러내용을 바인딩하기 위해 사용되낟
