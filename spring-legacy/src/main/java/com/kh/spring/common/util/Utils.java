...
public class Utils {
	 ...
	 * XSS(크로스 사이트 스크립트 공격)
	 * - 스크립트 삽입공격
	 * - 사용자가 스크립트 태그를 게시글에 작성하여 클라이언트가 게시글 클릭시
	 * 스크립트에 지정된 코드가 실행되도록 유도하는 방식
	 * - 위 냐용을 db에저장 후 브라우저에 렌더링하면 문제가 발생ㅎ=항수있는이
	 * 태그가 아닌 기본 문자열로 인식할 수 있게끔 thml내부 entity로 변환처리를수해앻
	 * 
	 * 
	 * 
	 * 
