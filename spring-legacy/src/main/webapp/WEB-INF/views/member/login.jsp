...
                </div>
				<!-- 
					...
					form:form태그는 csrf인증 토큰을 input type hidden으로 자동생성해준다.
					
					CSRF(Cross-site Request Forgery)
					 - 로그인 된 사용자의 브라우저 세션을 몰래 탈취하여 공격자가 사용자인 것처럼 요청을 
					   서버로 보내는 공격기법.
					 - 세션에 인증정보를 보관하는 경우, 세션은 브라우저 단위로 저장되므로 하나의 
					   브라우저에서 해커의 웹사이트와 정상적인 사이트 동시에 로그인하는 경우 해커의 웹사이트에서도
					   정상사이트의 세션데이터를 이용할 수 있다.
					 - csrf는 서버측에서 생성한 랜던토큰으로, 
					   스프링 시큐리티는 이 토큰이 있는 요청만 유효한 요청으로 간주한다.
				 -->
